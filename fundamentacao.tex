...
obtidos com o CFCM, sobre a coleção de documentos presentes no nó a ser expandido. 

Seja então $D = \{d_1,d_2,...,d_n\}$ um conjunto com $n$ documentos, para produzir o primeiro nível
da hierarquia o método HFCM executa o algoritmo FCM sobre $D$, produzindo uma partição 
fuzzy $U_{c \times n}[1]$ e os protótipos 
$V[1] = \{v_1[1],v_2[1],...,v_c[1]\}$, onde $[1]$ representa o nível da 
hierarquia\cite{Nogueira2013}, cuja largura da até o momento corresponde a quantidade de grupos $c$.

A decisão de expandir um determinado grupo presente nas folhas da árvore, se dá pela avaliação do
agrupamento, que é realizado através do índice de desempenho $Q$ (\ref{eq:qindex1})(\ref{eq:qindex2})
, tal que o índice $Q$
avalia a qualidade dos protótipos gerados pelo agrupamento. De maneira que quanto melhor for um 
...
$l$ da hierarquia e
\begin{equation}
  D_j[l] = \left\{d_{k} | \mu(d_{k}, g_j[l]) \leq \frac{\frac{1}{l}}{c}\right\}
\end{equation}
...
ou com a estabilização das medidas de desempenho dos grupos.  
...
Para o nível inicial da hierarquia a função de pertinência é a mesma do FCM (\ref{eq:pertinencia}),
porém para os níveis mais abaixo a função de pertinência é redefinida como sendo
\begin{equation}
  \mu(d_i,g_h[l])[j,l] = \frac{\mu(d_i,g_j[l-1])}
  {\sum_{k=1}^c \left(\frac{dist(d_i,v_h[l])}{dist(d_i,v_k[l])}\right)^{\frac{1}{m-1}}}, 
  1 < h \leq c,
  d_i \in D_j[l-1]
  \label{eq:pertcfcm}
\end{equation}
, cujo o valor de $l$ corresponde ao nível da hierarquia e $j$ seja o grupo expandido no nível $l-1$.
...
% DEFINIR AS CONSTRAINTS DO HFCM
