...
\end{equation}
, 
tal que $v_i$ seja o protótipo de $g_i$ e $c$ o número de grupos gerados no agrupamento. Sendo $m$ 
...
O algoritmo FCM realiza então uma série de atualizações nos protótipos dos grupos, a partir de
uma uma pseudo partição fuzzy.
O termo pseudo partição é empregado pois os elementos em uma partição fuzzy, não estão associados
a um único grupo. Assim, os valores de $v_1,v_2,v_3,...,v_c$ são comumente inicializados
...
realizada a partir da equação (\ref{eq:prototipos}). Onde por sua vez o grau de
pertinência $\mu(d_i,g_k)$ de cada documento na coleção, se dá por
\begin{equation}
...
\end{equation}
. Sendo no contexto do agrupamento de 
coleções textuais $dist(d_i,g_k) = scos(d_i,g_k)$.

Sendo assim 
A restrição (\ref{eq:pert_restricao_2}) da equação 
(\ref{eq:pertinencia}) tem como função evitar que algoritmo FCM produza grupos vazios. Enquanto a
equação (\ref{eq:pert_restricao_1}) impõe que a soma das pertinências seja sempre igual a um. 
Porém isto gera um problema em elementos equidistantes aos grupos. Ou seja, quando temos a caso 
...
\begin{figure}[!htp]
  ...
  \caption{Problema dos elementos equidistante do algoritmo FCM. Na imagem $g_1$ e $g_2$ são grupos,
