...
O algoritmo FCM realiza uma série de atualizações nos protótipos $G = \{g_1,g_2,g_3,...g_n\}$ 
dos grupos, a partir de uma uma pseudo partição fuzzy, tal que $n$ seja o total de grupos. 
O termo pseudo partição é empregado pois os elementos em uma partição fuzzy, não estão associados 
a um único grupo. Os valores de $g_1,g_2,g_3,...g_n$ são comumente inicializados 
aleatoriamente\cite{Nogueira2013} ou com o resultado de outro agrupamento previamente 
...
definido na equação (\ref{eq:pertinencia}), onde no contexto do agrupamento de documentos $dist(d_i,g_k) = scos(d_i,g_k)$.

\begin{equation}
  \mu(d_i,g_k) = \frac{1}{\sum_{j=1}^n(\frac{dist(d_i,g_k)}{dist(d_i,g_j)})^{\frac{1}{m-1}}}
  \label{eq:pertinencia}
\end{equation}

O método FCM sujeita a equação (\ref{eq:pertinencia}) as restrições (\ref{eq:pert_restricao_1})
(\ref{eq:pert_restricao_2}), o que impões limitações ao método.
\begin{equation}
